...
with st.sidebar:
    ...
    st.divider()
    if st.button("🔄 Rescan Filtered Feed", use_container_width=True):
        st.cache_data.clear()
...
with tab_screener:
    ...
    with k1: st.markdown(f"<div class='kpi-card'><div class='kpi-title'>Verified Matches</div><div class='kpi-value'>{total_m}</div></div>", unsafe_allow_html=True)
    ...
    with col_action:
        ...
        if len(selectable_stocks) > 0:
            ...
            st.warning("Loading verified data...")
